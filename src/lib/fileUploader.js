...
class FileUploader {
    ...
    addToQueue(files) {
        ...
        files.forEach((file) => {
            ...
            if (uploader._isValidFile(fileAlias, uploader.filters)) {
                ...
                uploader.onAddFileFail(
                    file,
                    uploader.filters[uploader.failFilterIndex],
                );
            }
        });
    ...
    _iframePost(filecontrol) {
        ...
        form.abort = () => {
            ...
            uploader._onAbortUpload(
                filecontrol,
                xhr.response,
                xhr.status,
                headers,
            );
            uploader._onCompleteUpload(
                filecontrol,
                xhr.response,
                xhr.status,
                headers,
            );
            uploader.vm.$refs.fileInput.parentNode.parentNode.appendChild(
                input,
            );
            uploader.vm.$refs.fileInput.parentNode.removeChild(form);
        };

        uploader.vm.$refs.fileInput.parentNode.insertBefore(
            form,
            uploader.vm.$refs.fileInput,
        );
